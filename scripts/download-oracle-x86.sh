...
# 清理安装包和临时文件
rm -rf /var/lib/apt/lists/* instantclient-sdk-linux.x64-19.23.0.0.0dbru.zip instantclient-basic-linux.x64-19.23.0.0.0dbru.zip instantclient-sqlplus-linux.x64-19.23.0.0.0dbru.zip
apt -y clean
apt -y remove wget unzip
apt -y autoremove
rm -rf /var/cache/apt

echo "X86 Oracle Instant Client installation is complete."
